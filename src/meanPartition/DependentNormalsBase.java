...
/**
 * This class provides a multidimensional base distribution with dependent dimensions, 
 * where the cluster likelihood is described by a multivariate normal distribution with parameters coming 
 * from a normal-inverse-Wishart distribution.
 * 
 * @author Thomas J. Glassen
 */

//this import is from the package ojAlgo: http://ojalgo.org/
...
public class DependentNormalsBase extends BaseDistribution{
	...
	public DependentNormalsBase(double[] priorMu, double[][] priorPsi, double priorKappa, double priorNu, double[][] observations) {
		...
		//old version with native java
		m = new Matrix(priorPsi);
		m.multiplyWith( (priorKappa + 1) / ( priorKappa * (priorNu - nDims + 1) ));
		tempDetSigma = m.det();
		priorInvSigma = m.inv();
		
		//new version using ojalgo
//		PrimitiveMatrix mtrx = PrimitiveMatrix.FACTORY.rows(priorPsi).multiply( (priorKappa + 1) / ( priorKappa * (priorNu - nDims + 1) ));
//		tempDetSigma = mtrx.getDeterminant().doubleValue();
//		PrimitiveMatrix mtrxI = mtrx.invert();
//		priorInvSigma = new double[nDims][nDims];
//		for(int i = 0; i < nDims;i++)
//			for(int j = 0; j < nDims;j++)
//				priorInvSigma[i][j] = mtrxI.doubleValue(i,j);
		
		priorLLSum = logGamma((priorNu + nDims) / 2) - (logGamma(priorNu / 2) + Math.log(priorNu * Math.PI) * nDims/2 + Math.log(Math.abs(tempDetSigma)) * 0.5);
	...
	protected void setNewClusterForObservation(int newClusterIndex, int observationIndex){
		...
		if(n > 0) {			
			...
			if(nDims == 2) { //if we have only 2 dims then calculate determinant and inverse directly
				...
				//new version using ojalgo
//				PrimitiveMatrix mtrx = PrimitiveMatrix.FACTORY.rows(postPsiStar);
//				tempDetSigma = mtrx.getDeterminant().doubleValue();
//				PrimitiveMatrix mtrxI = mtrx.invert();
//				for(int i = 0; i < nDims;i++)
//					for(int j = 0; j < nDims;j++)
//						postInvSigma[index][i][j] = mtrxI.doubleValue(i,j);
			}
	 ...
	 * https://en.wikipedia.org/wiki/Conjugate_prior#Continuous_distributions
	 */
	public double[] getClusterLogLikelihoods(int observationIndex, int[] clusterIndices) {
		...
		for(int cluster = 0;cluster<clusterIndices.length;cluster++){
			...
			likelihoods[cluster] = logDensityInMultivariateTDistibution(observations[observationIndex], postNu[i] - nDims + 1, postMu[i], 
	 ...
	 */		
	private double logDensityInMultivariateTDistibution(double[] x, double nu, double[] mu, double lLSum, double[][] invSigma) {
		return lLSum + Math.log(1 + multipyDevVectorTMatrixAndDevVector(x,mu, invSigma) / nu) * (nu + nDims) / -2;
	}
